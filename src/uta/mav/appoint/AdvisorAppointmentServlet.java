...
/**
 * Servlet implementation class AdminAdvisingServlet
 */
@WebServlet("/AdvisorAppointmentServlet")
public class AdvisorAppointmentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (advisorUser == null) {
				...
				int tempType = advisorUser.getDegType();
				degreeType = new ArrayList<>();
//				if (tempType >= 4) {
//					tempType -= 4;
					degreeType.add("Doctorate");
...
//
			...
			//if (advisors.size() != 0) {
				session.setAttribute("advisors", advisorUser);
			//}
		...
		request.getRequestDispatcher("/WEB-INF/jsp/views/advisorbookappointment.jsp").forward(request, response);
	}
		...
		try {
			...
			if (advisor != null && !advisor.equals("all")) {
				...
				for (int i = 0; i < advisors.size(); i++) {
					...
					if (advisors.get(i).advisesStudent(tempDep, tempMajor, null, degreeValue)) {
