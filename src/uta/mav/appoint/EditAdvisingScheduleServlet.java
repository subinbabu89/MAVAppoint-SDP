	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (user != null) {
			...
			try {
				...
				at.setEmail(user.getEmail()); // using pname to find correct
												// advisor instead of email
...
//		request.setAttribute("includeHeader", header);
		// request.getRequestDispatcher("/WEB-INF/jsp/views/login.jsp").forward(request,
		// response);
	}
	...
	private void insertSchedule(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			char[] tempValue = RandomPasswordGenerator.generatePswd(minLen, maxLen, noOfCAPSAlpha, noOfDigits,
					noOfSplChars);
			...
			for (int i = 0; i < rep; i++) {
				char[] tempValue2 = RandomPasswordGenerator.generatePswd(minLen, maxLen, noOfCAPSAlpha, noOfDigits,
						noOfSplChars);
		...
		try {
			...
			for (Iterator iterator = appointments.iterator(); iterator.hasNext();) {
				Appointment object = (Appointment) iterator.next();
				String studentEmail = object.getStudentEmail();

				System.out.println("Student Email " + studentEmail);
				String msgSub = "Mavappoint User Information";

				String msgText = "Advisor has editted the time slot for his advising schedule. Please log in to check the new schedule";
				String toEmail = studentEmail;
